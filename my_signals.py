...
def mysaw(frequency=0.5, start_time=-3.0, end_time=3.0, amplitude=1.0, sample_rate=1000, symmetry=0.0):
    ...
    sample_rate : int
        Samples per second.
    symmetry : float
        Fraction in [0,1]; 0.0 rising ramp, 1.0 falling ramp; intermediate piecewise.

    Returns
    -------
    t : ndarray
        Time array from start_time to end_time (endpoint excluded).
    y : ndarray
    ...
    if s == 0.0:
        ...
        up = phase < s
        y[up] = -1.0 + 2.0 * (phase[up] / s)
        y[~up] = 1.0 - 2.0 * ((phase[~up] - s) / (1.0 - s))

    # KEY FIX: Normalize to exact [-1, 1] on sampled points
    y_range = np.ptp(y)  # max - min
    if y_range > 0:
        y = 2.0 * (y - np.min(y)) / y_range - 1.0
